...
def top10_followers(database, eras):
    ...
    df = pd.read_sql_query(f"""
        SELECT ar.name, MAX(ar.followers) as followers 
        FROM artist_data ar 
        JOIN albums_data al ON ar.id = al.artist_id 
        WHERE era IN ({eras_str}) 
        GROUP BY ar.name""", database
    )
    ...
    short_names = [
        ...
        for name in top10_followers['name']
    ]
...
def top10_popularity(database, eras):
    ...
    df = pd.read_sql_query(f"""
        SELECT ar.name, MAX(ar.artist_popularity) as artist_popularity 
        FROM artist_data ar 
        JOIN albums_data al ON ar.id = al.artist_id 
        WHERE era IN ({eras_str}) 
        GROUP BY ar.name""", database
    ...
    short_names = [
        name[:15] + "…" if len(name) > 15 else name
        for name in top10_popularity['name']
    ]
...
def linear_regression(database, eras):
    ...
    df = pd.read_sql_query(f"""
        SELECT MAX(ar.followers) as followers, MAX(ar.artist_popularity) as artist_popularity 
        FROM artist_data ar 
        JOIN albums_data al ON ar.id = al.artist_id 
        WHERE era IN ({eras_str}) GROUP BY ar.name""", database
    )
    ...
    ax.plot(
        sorted_followers,
        intercept + slope * np.log1p(sorted_followers),
        color='#7CFC00'
    )
